...
describe('SubscriptionsManagementService', () => {
    ...
    describe('createSubscription', () => {
        ...
        it.skip('should handle other errors when creating subscription', async () => {
            ...
            await expect(subscriptionsService.createSubscription(validSubscriptionParams)).rejects.toThrow('Azure error handled');
            expect(mockLogger.error).toHaveBeenCalledWith('Failed to create subscription', error);
    ...
    describe('getSubscription', () => {
        ...
        it.skip('should handle other errors when getting subscription', async () => {
            ...
            await expect(subscriptionsService.getSubscription('sub1')).rejects.toThrow('Azure error handled');
            expect(mockLogger.error).toHaveBeenCalledWith('Failed to get subscription', error);
            expect(mockAzureClient.handleAzureError).toHaveBeenCalledWith(error);
        });
    });
});